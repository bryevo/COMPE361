...
namespace Assignment3
{
    ...
    public class Words
    {
        ...
        public string[] MorphWord(string morph, string alphabet)
        {
        ...
        public void MorphChain(string start, string end, int max)
        {
            ...
            List<string[]> totalChains = new List<string[]>();
            List<string> setOne = new List<string>(morphSet.MorphWord(start, alphabet));      // told:{bold, cold, fold, gold, hold, sold}
            ...
            while (counter != max)
            {
                while (!foundWords.Contains(end))
                {
                    ...
                    for (int i = 0; i < totalChains.Count; i++) // told:{bold, cold, fold, gold, hold, sold}
                    {
                        string[] chains = totalChains[i];
                        for (int j = 0; j < chains.Length; j++) // bold: {bald, cold, sold}
                        {
                            var count = end.Count(foundWords.Contains); //LINQ
